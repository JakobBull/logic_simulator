...
def test_query_output(
    used_names, new_names, name_id, expected_string, name_string_list
):
...
def test_lookup_finds_stored_names(new_names, used_names, name_string_list):
    # tests that the right name_ids list is returned for the example
    # name_string_list
    # all name ids should be found when using the example name_string_list
    # on the used_names instance
    assert used_names.lookup(name_string_list) == [0, 1, 2]
    # tests that no name ids are found when list of names that haven't
    # been used before are used as parameter to lookup function for both
    # new_names and used_names instances
...
def test_get_name_string_raises_exceptions(used_names):
    ...
    with pytest.raises(ValueError):  # only accepts +ve integers
        used_names.get_name_string(-1)  # can't have -ve index for names list
...
def test_get_name_string_output(
        used_names, new_names, name_id, expected_string):
    # tests that get_name_string returns the right name when given the
    # corresponding name_id for instance of name, used_names, that has
    # seen all of these names to be tested
